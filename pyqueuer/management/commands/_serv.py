...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        #tmp.writelines(['python manage_with_env.py %s %s' % (venv, cmd), line_break])
        tmp.writelines(['python manage.py %s' % cmd, line_break])
        #tmp.close()

        arguments = [tmp.name, ]

        p = Popen(arguments, executable=exe, stdout=PIPE, stderr=PIPE,
                             stdin=PIPE, env=os.environ.copy(), universal_newlines=True)
